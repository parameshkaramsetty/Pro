...
void fun(int i , int n){
    if(i>n) return ;//stops the function call // 6 > 5
    cout<<i<<" ";//1
    fun(i+1,n);//2 3 4 5
...
void print_name(int i,int n,string s){
    if(i>n){ // i==n+1 can also be written as i>n
        return ;
    }
    cout<<i<<")"<<s<<"\n";
    print_name(i+1,n,s);
...
bool printOS(int a[],int i , int n , int k , vector<int>&s,int sum){
    ...
    s.push_back(a[i]);
    sum+=a[i];
    if(printOS(a,i+1,n,k,s,sum)){// if it is true
...
*/
    // base case
    if(sum>k)return 0; // condition not satisfied - only if elements are non-negative integers
    if(i>=n){
        if(sum==k){
